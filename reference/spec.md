...
| `tools/run_post_scan.py::classify_band` + thresholds 30/20/10 | `modal_workers/shared/rubric_engine.py::classify_band` | Live Conan v2 authoritative contract. | **Update 2026-05-06 (D-035):** v2 adopted the 30/20/10 shift originally pioneered in v1 D-034. Both `rubric_engine.py` and `_shared/convergence.ts` enforce 30/20/10. The legacy `engine/` folder remains the historical reference. |
...
skill thesis_writer():
    ...
    for job in jobs:
        ...
        for attempt in 1..2:
            ...
            if thesis.confidence == "low" or thesis.insufficient_signal:  promote_flagged(thesis, job); break
...
If `confidence == "low"` OR the routine returns a structured `insufficient_signal` response, the job skips both gates and **promotes flagged**: a `state='watch'` candidate is created with `extensions.routine_declined=true` + `extensions.routine_decline_reason=<reason>`, and `thesis_jobs.status='promoted'` with `gate_reasons=['routine_declined_flagged']`. The flagged-pass does NOT consume the 15/day promotion quota, does NOT fire the fanout email (the fanout edge function filters `payload.routine_declined === true`), and is excluded from the watch→active auto-promote rule (§7.5). Operators see the row with a "routine declined" pill in the dashboard candidates board and can re-draft (which strips the flag) or kill manually. The v2 schema intentionally makes `confidence: "low"` the honest path when a routine can't meet the steelman + web_research + tagging bar; that's preferred over hedging prose that passes char counts but fails asymmetry, AND preferred over a hard DLQ that hides the row from human review. See `.claude/skills/thesis_writer.md` §6.5 + §8c-flagged for full SQL.
...
- Refusal contract: when the signal lacks enough primary-source substance for a specific thesis, the skill must set `confidence: "low"` or `insufficient_signal: true` and **promote flagged** (§7.4 honest-decline path; `extensions.routine_declined=true`, `state='watch'`, no email, no quota) rather than produce hedging prose. The flagged-pass keeps the row visible to operators and is preferred over a hard DLQ that hides the AI's decision.
...
2. **Mechanical date sweep first (cheap, deterministic, no Claude call).**
   - **Promote watch → active (2026-04-22 amendment; routine_declined gate added 2026-04-27).** If `state='watch'` AND `extensions->>'routine_declined' IS DISTINCT FROM 'true'` AND a catalyst lands in the next 60 days — either `next_catalyst_date BETWEEN now() - interval '7 days' AND now() + interval '60 days'` OR `next_catalyst_window && tstzrange(now(), now() + interval '60 days', '[]')` — then set `state='active'`, INSERT `candidate_events(event_type='state_changed', payload={from:'watch', to:'active', reason:'catalyst_within_60d', source:'candidate_aging', stage:'A'})`. Challenger thesis approval is implicit (every non-flagged `watch` row already passed thesis_writer's challenger, §7.4), so catalyst proximity is the only mechanical promotion gate; flagged-pass rows (§7.4 routine_declined short-circuit) skip both gates and must wait for operator re-draft or kill before they can promote. First-match-wins ordering places this above the aged-out rule so a 60-day-stale watch candidate with a newly-near catalyst is promoted rather than killed.
   ...
   - If `state='active'` AND `updated_at < now() - interval '30 days'` AND (`next_catalyst_date IS NULL` OR `next_catalyst_date > now() + interval '60 days'`) AND (`next_catalyst_window IS NULL` OR NOT (`next_catalyst_window && tstzrange(now(), now() + interval '60 days', '[]')`)) → demote to `state='watch'`. The 60-day catalyst threshold is symmetric with the promote rule above — asymmetric thresholds caused active/watch oscillation in the original (30-day) formulation.

**thesis_writer alignment (§7.4).** The same 60-day catalyst-proximity rule is applied inline when `thesis_writer` UPSERTs a new candidate: initial `state='active'` if a catalyst is within 60 days at draft time, else `state='watch'`. Most eligible candidates thus land in `active` on day one without waiting for the next aging sweep. See [thesis_writer.md step 7](../.claude/skills/thesis_writer.md). Re-drafts (convergence path) do NOT recompute state — `candidate_aging` owns all post-creation transitions.
...
| Routine declines (`confidence:"low"`) | Routine returns structured refusal | Skip both gates and **promote flagged**: candidate UPSERT with `state='watch'` + `extensions.routine_declined=true`; `thesis_jobs.status='promoted'`, `gate_reasons=['routine_declined_flagged']`. Does not consume daily quota, does not fire email, blocked from watch→active auto-promote. Operator reviews the flagged row in the dashboard. |
...
- **D-034's 30/20/10 thresholds are live policy as of D-035 (2026-05-06).** Both `rubric_engine.py` and `_shared/convergence.ts` enforce the shifted bands across the Modal + Supabase runtime; the FDA scanner's `BAND_THRESHOLDS_DEFAULT` was shifted in lockstep. The legacy `engine/` folder remains the historical reference for the original D-034 experiment.
...
- **`binary_catalyst`** now gets low-risk market snapshot fields (`adv_usd`, `market_cap_usd`) plus a narrow mechanical `competitive_landscape` proxy from `approval_history_count` / `enrichment.fda_history`. It still lacks consistently auditable market-implied mispricing inputs, so `binary_catalyst.market_mispricing` remains resolver-weighted and `binary_catalyst.ev_floor` remains much more meaningful on some sources than others.
...
- **`takeover_candidate`** now gets shared market snapshot support for `adv_usd`, `market_cap_usd`, and a coarse `valuation_cushion_pct` price-vs-5y-median proxy on ticker-backed rows. A fully honest valuation workflow still needs deeper EV / revenue / EBITDA history plus a stable sector-consolidation signal; the methodology's deferred watchlist cap for active-consolidation sectors remains intentionally unimplemented until those payloads exist.
- **Dashboard review surfaces** now need to track both halves of the queue contract: `thesis_jobs.status IN ('needs_scoring','scoring')` (resolver backlog) and `status IN ('queued','drafting','gate_failed_retrying')` (thesis backlog). Treat them as separate operational states, not one generic “thesis jobs” number.

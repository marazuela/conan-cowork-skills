...
2. **Mechanical date sweep first (cheap, deterministic, no Claude call).**
   - **Promote watch → active (2026-04-22 amendment).** If `state='watch'` AND a catalyst lands in the next 60 days — either `next_catalyst_date BETWEEN now() - interval '7 days' AND now() + interval '60 days'` OR `next_catalyst_window && tstzrange(now(), now() + interval '60 days', '[]')` — then set `state='active'`, INSERT `candidate_events(event_type='state_changed', payload={from:'watch', to:'active', reason:'catalyst_within_60d', source:'candidate_aging', stage:'A'})`. Challenger thesis approval is implicit (every `watch` row already passed thesis_writer's challenger, §7.4), so catalyst proximity is the only promotion gate. First-match-wins ordering places this above the aged-out rule so a 60-day-stale watch candidate with a newly-near catalyst is promoted rather than killed.
